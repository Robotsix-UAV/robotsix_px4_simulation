...
"""
Complete simulation launch file for Mavlink
Handles Gazebo, model spawning, and PX4 instances with Mavlink (no XRCE agent)
"""
...
def launch_simulation(context):
    ...
    # 3. Start PX4 instances for each model with Mavlink
    px4_exec = os.path.join(px4_dir, 'bin', 'px4')
    
    # Base mavlink UDP port (increments for each instance)
    ...
    for idx, model in enumerate(models):
        ...
        # Environment variables for PX4 with Mavlink
        px4_env = {
            'PX4_SYS_AUTOSTART': str(model['px4_parameters']),
            'PX4_GZ_MODEL_NAME': model['model_name'],
            # Disable XRCE-DDS for mavlink mode
            'PX4_UXRCE_DDS_DISABLE': '1'
        }
    ...
    # Note: No MicroXRCE-DDS Agent for Mavlink mode
